<x-rpd::view title="Subscription {{ $subscription->shortId }}">
    <x-slot name="buttons">
        ...
        <a href="{{ route_lang('subscriptions.table') }}" class="btn btn-outline-primary">{{ __('List') }}</a>
    </x-slot>
        ...
        <div class="col-md-8">
            ...
            <x-rpd::card title="Subscription items">
                ...
                <table class="table table-sm">
                    ...
                    <tfoot>
                    <tr class="tr-small"><td colspan="4"></td><td class="text-end">{{ __('Subtotal') }}</td><td class="text-end">{{ number_format($subscription->subtotal, 2) }} {{ Cart::currency() }}</td></tr>
                    <tr class="tr-small"><td colspan="4"></td><td class="text-end">{{ __('Tax') }}</td><td class="text-end">{{ number_format($subscription->tax, 2) }} {{ Cart::currency() }}</td></tr>
                    <tr><td colspan="4"></td><td class="text-end"><strong>{{ __('Total') }} / {{ $subscription->period === 'yearly' ? __('year') : __('month') }}</strong></td><td class="text-end"><strong>{{ number_format($subscription->total, 2) }} {{ Cart::currency() }}</strong></td></tr>
                    </tfoot>
                </table>
            </x-rpd::card>
            ...
            @if($hasRecorder)
                <x-rpd::card title="Payments">
                    @if(count($payments) === 0)
                        <div class="text-muted small">{{ __('No payment recorded yet.') }}</div>
                    @else
                        <table class="table table-sm">
                            <thead><tr><th>{{ __('Payment') }}</th><th>{{ __('Description') }}</th><th>{{ __('Gateway') }}</th><th>{{ __('Status') }}</th><th>{{ __('Date') }}</th><th class="text-end">{{ __('Total') }}</th><th></th></tr></thead>
                            <tbody>
                            @foreach($payments as $payment)
                                <tr wire:key="sp-{{ $payment->id }}">
                                    <td>@if(Route::has('payments.view'))<a href="{{ route_lang('payments.view', $payment) }}">{{ $payment->shortId ?? substr($payment->id, 0, 8) }}</a>@else{{ substr($payment->id, 0, 8) }}@endif
                                        @if($payment->ref_subscription_id)<span class="badge bg-light text-dark">{{ __('renewal') }}</span>@else<span class="badge bg-light text-dark">{{ __('first') }}</span>@endif</td>
